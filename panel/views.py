...
def add_booking(request, book_id):
    stud = Student.objects.get(user = User.objects.get(id = request.user.id))
    fullnames = list(Student.objects.all().values_list('fullname', flat=True))
    book = Book.objects.get(id = book_id)
    data = {'fullname': stud.fullname, 'book': book, 'fullnames': fullnames}
    if request.method == 'POST':
...
def wallet_view(request):
    stud = Student.objects.get(user = User.objects.get(id = request.user.id))
    borrow_fine = list(Borrow.objects.all().values_list('fine_amount', flat=True))
    total_fine = sum(borrow_fine)
